...
### 📌 레이아웃
- [X] 중앙 정렬, 다크 테마 카드(모달)로 표시된다.
- [X] 상단에 서비스명 “Prompt_LAB”을 표시한다.
- [X] 입력 필드: ID(이메일) / Password 가 세로로 배치된다.
- 필드 하단에 검증 메시지를 표시한다(붉은색, 아이콘 포함).
  - [X] 주요 버튼: Sign In 1개.
  - [X] 하단 링크: Sign Up / Find ID / Find Password.

### 📌 입력/포맷
- [X] ID는 이메일 형식만 허용한다.
- [X] Password는 마스킹 처리된다(표시/숨김 토글 선택).
- [X] Enter 키로 로그인 제출이 가능하다.

### 📌 검증(클라이언트)
- [X] ID/Password 미입력 시 각각 메시지를 표시한다.
- [X] ID가 이메일 형식이 아니면 오류 메시지를 표시한다.
- [X] 비밀번호 조건을 만족하지 않으면 안내한다.

### 📌 검증(서버/인증 결과)
- [X] 아이디 또는 비밀번호 불일치 시 오류 메시지를 표시한다.
- [X] 서버 오류 시 일반 오류 메시지(재시도 안내)를 표시한다.
...
- [X] 모달 바깥 클릭/ESC로 닫기 가능(로그인 진행 중엔 닫기 불가 옵션).
...
### 📌 레이아웃
- [X] 중앙 모달에 제목 Sign Up 표시, 닫기(ESC/바깥 클릭) 가능
- [X] 입력 필드 세로 배치, 각 필드 하단에 오류 메시지 영역

### 📌 입력 필드
- ID (이메일)
  - [X] 필수, 이메일 형식
- Password
  - [X] 필수, 비밀번호는 최소 8자 이상이어야 하며, 대문자, 소문자, 숫자, 특수문자를 각각 최소 1개 이상 포함해야한다.
- Password Confirm
  - [X] 필수, Password와 일치해야 함
- Name
  - [X] 필수, 2–30자
- Team Code (optional)
  - [X] 선택 입력, 유효한 팀 코드 형식/존재 여부 확인

### 📌 검증/에러
- [X] 미입력 또는 형식 불일치 시 해당 필드 아래 오류 메시지 표시
- [X] 이미 등록된 이메일이면 중복 오류 표시

### 📌 동작
- [X] Sign Up 클릭 또는 Enter → 유효성 검사 후 계정 생성
- [X] 계정 생성 성공 시 자동 로그인 및 모달 닫기 → (팀 코드가 있다면) 해당 팀에 자동 참가
...
### 📌 상단 정보 영역
- [X] 프로젝트명, 설명, 팀코드 복사 버튼 표시
- [ ] 리더 이름과 팀원 목록 표시, New member 버튼으로 팀원 추가 가능

### 📌 단계 네비게이션
- [X] 상단에 Test Cases → Experiments → Results 단계 표시
- [X] 현재 단계는 강조 표시되며, 클릭으로 단계 이동 가능

### 📌 Test Cases 단계
- [X] 테스트 케이스 목록 테이블: Request, Result 컬럼 포함
- [ ] Import cases with CSV 버튼으로 케이스 일괄 등록 가능
- [X] New case 버튼으로 개별 케이스 추가 가능

### 📌 Experiments 단계
1) 단일 실험 (Single Experiment)
- [ ] 모델, 프롬프트, 외부 문서(.docs, .xlsx, .pdf) 연결 가능
- [X] 하단 테이블에 Request, Expected Result, Experiment Result 표시
